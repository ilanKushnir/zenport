...
export function renderCatalog(entries: CatalogEntry[]): string {
  ...
  for (const e of entries) {
    ...
    const total = i.totalDurationSec
      ...
      : 'length unknown';
    const progress = i.completedCount > 0 ? `, ${i.completedCount}/${i.trackCount} done` : '';
    lines.push(
...
export const PLAN_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  required: ['name', 'intention', 'summary', 'practice', 'learning', 'outline'],
  properties: {
...
export function planPrompt(req: AiPlanRequest, catalog: string): { system: string; user: string } {
  const system = [
  ...
  const user = [
    ...
    `Length: ${req.weeks} week${req.weeks === 1 ? '' : 's'}, starting ${req.startDate}.`,
    req.practice
    ...
    '',
    'Catalogue (handle | type | creator > series > title | tracks, length, progress; lessons listed beneath):',
    catalog,
  ]
    .filter(Boolean)
    .join('\n');
