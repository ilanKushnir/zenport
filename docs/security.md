...
- **Spending as little as the answer needs.** Requests carry the library written compactly (grouped by creator and series, numbered parts as ranges). Thinking models are asked for low effort, or medium for a whole plan. Web searches pull in the short version of each page. Simple jobs (levels, fixes, finding pictures, today's pick) run on the cheaper sibling of the chosen model: its mini, Haiku or Flash, when the key has one, and the chosen model if not. Levels and fixes remember what they have read (a fingerprint of each recording as it was), so later runs send only what is new or changed. A run over a settled library makes no request at all. If a provider turns one of these settings down, the request is asked again without it.
