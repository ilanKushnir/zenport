...
- **No telemetry.** Outbound calls are exactly: the three user-initiated YouTube/transcription cases in [youtube-sources.md](youtube-sources.md) and [configuration.md](configuration.md), the AI provider a person connected (when they use a feature), and the picture fetches above.
