...
- **No telemetry.** Outbound calls are exactly: the three user-initiated YouTube/transcription cases in [youtube-sources.md](youtube-sources.md) and [configuration.md](configuration.md), the AI provider a person connected (when they use a feature), the picture fetches above, and Discover's link checks: each recommended link is opened once, under the same https-and-public-addresses-only guard, before it is shown.
