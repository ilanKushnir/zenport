...
- **Offline meditations** — download a meditation (courses and talks stay online; they are too large) and it plays - and seeks - with no connection, straight from the device. The meditation page shows the size, the progress and _On this device_; a Downloads page lists everything kept, the space used and free, and removes one or all. The app opens offline as the last person signed in, and sits played offline are sent up when the connection returns, so streaks and counts stay true.
- **Practise together** — no public sign-up: an admin sends a single-use invitation link, chooses whether the newcomer is a member or an admin, and can make them friends on the way in. Friends see each other's day (a ring once you have sat, "sitting now", streaks and the days you both practised), send a bow for a sit, a gentle nudge once a day, or an invitation to sit with a particular recording. Each person chooses how much friends see: everything, just the numbers, or nothing.
- **Planning with AI, in stages** — with your own AI - OpenAI, Anthropic, Google Gemini, OpenRouter or your own OpenAI-compatible server (or the one the owner shares), describe what you want and how learning and practice should fit together - side by side, learn first, or taking turns - and get a path of dated stages built only from your library and what you have already done, as long as it needs to be.
